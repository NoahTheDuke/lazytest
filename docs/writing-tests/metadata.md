...
All of the test suite and test case macros ([`defdescribe`][defdescribe], [`describe`][describe], [`it`][it], [`expect-it`][expect-it]) take a metadata map after the docstring. Adding `:focus true` to this map will cause *only* that test/suite to be run. Removing it will return to the normal behavior (run all tests).
...
(defdescribe metadata-test
  {:integration true}
  (it "something computationally expensive"
    (expect (= 1 2))))

(defdescribe metadata-test
  {:end-to-end true}
  (it "something-with-a-browser"
    (expect (= 1 2))))
```
